...
- 代理会話完了後、会話ログ全体を Mistral に送信し、以下の **5 軸** で評価する:
...
| 信頼性ファクター     | 自己申告と実際の行動（Interaction DNA）の一貫性          |


- 5 軸スコアを総合した多次元相性スコア（0〜100）を算出
- プロフィールベースのスコア（F3）と多次元相性スコア（F4）を統合した最終スコアを算出
- 最終スコア = プロフィールスコア × 0.4 + 多次元相性スコア × 0.6
